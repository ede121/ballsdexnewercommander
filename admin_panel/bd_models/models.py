...
class BattleRecord(models.Model):
    """Stores a simulated battle between two teams."""

    date = models.DateTimeField(auto_now_add=True, editable=False)
    team_a = models.JSONField(help_text="List of BallInstance ids and metadata for team A")
    team_b = models.JSONField(help_text="List of BallInstance ids and metadata for team B")
    log = models.TextField(help_text="Full battle log")
    winner = models.CharField(max_length=16, help_text="'A', 'B' or 'draw'")

    objects: Manager["BattleRecord"] = Manager()
    ...
    def __str__(self) -> str:
        return f"Battle #{self.pk} - {self.winner}"
